...
const actions = {
  ...
  async getGardensByLocation({ commit }, { longitude, latitude, radius }) {
    try {
      const gardens = await gardenService.fetchGardensByLocation(
        longitude,
        latitude,
        radius
      );
  ...
  async aggregateGardenPlants({ commit }, gardenId) {
    try {
      const numberOfPlants =
        await gardenService.aggregateGardenPlants(gardenId);
  ...
  async getGardensWithPagination({ commit }, { page, limit }) {
    try {
      const paginatedGardens = await gardenService.fetchGardensWithPagination(
        page,
        limit
      );
