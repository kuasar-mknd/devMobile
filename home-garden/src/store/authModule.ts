      ...
      if (axios.isAxiosError(error)) {
        ...
        switch (status) {
          case 400:
            commit(
              "setAuthError",
              "Mauvaise requête, l'utilisateur existe déjà."
            );
            break;
          case 488:
            commit(
              "setAuthError",
              "Un email valide est requis ou le mot de passe doit comporter au moins 6 caractères."
            );
            break;
          case 500:
      ...
      if (axios.isAxiosError(error)) {
        ...
        switch (status) {
          ...
          case 488:
            commit(
              "setAuthError",
              "Une adresse mail valide ou un mot de passe de 6 caractères minimum est requis."
            );
            break;
          case 500:
            commit("setAuthError", "Erreur interne du serveur.");
